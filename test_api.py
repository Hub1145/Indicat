...
def test_endpoints():
    ...
    r = requests.get(f"{BASE_URL}/indicators", headers=HEADERS)
    ...
    r = requests.post(f"{BASE_URL}/analyze/upload", json={"data": data, "include_history": True, "indicators": ["RSI", "MACD"]}, headers=HEADERS)
    ...
    r = requests.post(f"{BASE_URL}/confluence-score", json=payload, headers=HEADERS)
    ...
    r = requests.post(f"{BASE_URL}/analyze/mtf", json=mtf_payload, headers=HEADERS)
    ...
    r = requests.post(f"{BASE_URL}/analyze/correlation", json=corr_payload, headers=HEADERS)
    print(f"Status: {r.status_code}, Correlation: {r.json()}")

    # 7. Heatmap
    print("\nTesting /analyze/heatmap...")
    heatmap_payload = {"assets": ["BTC/USD", "ETH/USD"], "metric": "RSI"}
    r = requests.post(f"{BASE_URL}/analyze/heatmap", json=heatmap_payload, headers=HEADERS)
    ...
    r = requests.post(f"{BASE_URL}/is-trend-bullish", json=payload, headers=HEADERS)
    print(f"Status: {r.status_code}, Bullish: {r.json().get('bullish')}")

    # 10. Pattern Scan
    print("\nTesting /scan-patterns...")
    r = requests.post(f"{BASE_URL}/scan-patterns", json=payload, headers=HEADERS)
